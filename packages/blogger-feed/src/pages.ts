...
export class Pages extends Methods {
  ...
  async list(options: PagesListOptions = {}) {
    const { posts, pagination } = await this.client.request(`./pages/${options.summary === true ? 'summary' : 'default'}`, {
      params: options,
      exclude: ['query'],
    });
  ...
  async get(page_id: string, options: PagesGetOptions = {}) {
    ...
    const { posts } = await this.client.request(`./pages/${options.summary === true ? 'summary' : 'default'}/${encodeURI(page_id)}`, {
      exclude: ['query'],
    });

...
export class Client {
	...
	constructor(urlOrId: string | URL, options: ClientOptions = {}) {
		...
		if (
			...
			const origin =
				urlOrId instanceof URL ? urlOrId.origin : getOrigin(urlOrId);
			if (origin) {
	...
	get baseUrl() {
		...
		return input.type === "id"
			? `https://www.blogger.com/feeds/${input.value}/`
			: `${input.value}feeds/`;
	}
	...
	get blogId() {
		return (async () => {
			const { input } = this.options;
			this._blogId ??=
				input.type === "id" ? input.value : (await this.info).blog_id;
			return this._blogId;
		})();
	}
	...
	get blogUrl() {
		return (async () => {
			const { input } = this.options;
			this._blogUrl ??=
				input.type === "url" ? input.value : (await this.info).blog_url;
	...
	get bloggerBaseUrl() {
		return (async () =>
			`https://www.blogger.com/feeds/${await this.blogId}/`)();
	}

...
function assertValidSegment(segment: string): void {
	if (!VALID_SEGMENT.test(segment)) {
		throw new SyntaxError(
			`Invalid key segment '${segment}': characters [ and ] are not allowed`,
		);
	}
}
...
function assertUnblockedSegment(segment: string): void {
	if (BLOCKED_SEGMENTS.has(segment)) {
		throw new SyntaxError(
			`Disallowed key segment '${segment}': reserved prototype property`,
		);
	}
}
...
function assertValidKey(key: string): void {
	if (!VALID_KEY.test(key)) {
		throw new SyntaxError(
			`Invalid key '${key}': expected format is 'key', 'key[]', or 'key[segment]'`,
		);
	}
}
...
export function serialize<T extends URLSearchParams | FormData>(
...
export function deserialize(
	...
	for (const [rawKey, rawValue] of input.entries()) {
		...
		const value =
			typeof rawValue === 'string' ? parseValue(rawValue) : rawValue;
...
function setPath(
	target: Record<string, Serializable>,
	segments: string[],
	value: Serializable,
): void {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		if (segment === '') {
			...
			if (
				...
				Array.isArray(lastItem) !== shouldBeArray ||
				(nextSegment && nextSegment in lastItem)
			) {
				const next: Record<string, Serializable> | Serializable[] =
					shouldBeArray ? [] : {};
				current.push(next);
				current = next;
			} else {
			...
			if (Array.isArray(current)) {
				throw new TypeError(
					`Expected object at segment '${segment}' but got array`,
				);
			}
			...
			if (isLast) {
				if (segment in current) {
					...
					current[segment] = Array.isArray(existing)
						? [...existing, value]
						: [existing, value];
				} else {
			...
			if (segment in current) {
				const existing = current[segment];
				const isCompatible = shouldBeArray

...
export type Style = Partial<
	PickString<PickWritable<CSSStyleDeclaration>> &
...
export type UpdateElementOptions<T extends HTMLElement> = Partial<
	Omit<
		OmitFunction<PickWritable<T>>,
		'class' | 'attributes' | 'dataset' | 'style'
	>
> & {
	class?: string | string[];
...
export type CreateElementOptions<T extends HTMLElement> =
	UpdateElementOptions<T>;

export type CreateJavaScriptOptions = Omit<
	CreateElementOptions<HTMLScriptElement>,
	'textContent' | 'innerHTML' | 'innerText'
>;

export type CreateStylesheetOptions = Omit<
	CreateElementOptions<HTMLStyleElement>,
	'textContent' | 'innerHTML' | 'innerText'
>;

export type LoadJavascriptOptions = Omit<
	CreateElementOptions<HTMLScriptElement>,
	'src'
>;

export type LoadStylesheetOptions = Omit<
	CreateElementOptions<HTMLLinkElement>,
	'href' | 'rel'
>;
 ...
 */
function insertAfterendOrLastInHead<T extends Element>(
	element: T,
...
export function updateElement<T extends HTMLElement>(
	element: T,
	options: UpdateElementOptions<T>,
): T {
	if (options) {
...
export function createStylesheet(
	css: string,
	options?: CreateStylesheetOptions | ((element: HTMLStyleElement) => void),
): HTMLStyleElement {
...
export function loadJavascript(
	...
	if (!isString(url)) {
		return Promise.reject(
			new TypeError('Argument source must be URL or string'),
		);
	}
			...
			error: () => {
				...
				reject(
					new ResourceLoadingError(
						script,
						script.src,
						`Failed to load Javascript from ${script.src}`,
					),
				);
			},
		});
			...
			error: () => {
				...
				reject(
					new ResourceLoadingError(
						link,
						link.href,
						`Failed to load Stylesheet from ${link.href}`,
					),
				);
			},
		});

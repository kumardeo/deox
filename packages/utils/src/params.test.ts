...
describe('serialize', () => {
  test('serializes primitive values to URLSearchParams', () => {
    const params = serialize({ a: 'hello', b: 123, c: true, d: null }, new URLSearchParams());
  ...
  test('serializes Blob values only to FormData', () => {
    ...
    expect(() => serialize({ file: blob }, new URLSearchParams())).toThrow(TypeError);
  });
  ...
  test('throws for non-plain object input with exact error text', () => {
    expect(() => serialize(new Date() as any, new URLSearchParams())).toThrow(new TypeError("Argument 'input' must be a plain object"));
  });
  ...
  test('throws for invalid and reserved keys with exact error text', () => {
    ...
    expect(() => serialize({ '[a]': 'x' }, new URLSearchParams())).toThrow(
      new SyntaxError("Invalid key segment '[a]': characters [ and ] are not allowed"),
    );
    expect(() => serialize({ 'a[b': 'x' }, new URLSearchParams())).toThrow(
      new SyntaxError("Invalid key segment 'a[b': characters [ and ] are not allowed"),
    );
    expect(() => serialize(reserved as any, new URLSearchParams())).toThrow(
      new SyntaxError("Disallowed key segment '__proto__': reserved prototype property"),
    );
    expect(() => serialize({ a: { '[b]c]': 'x' } }, new URLSearchParams())).toThrow(
      new SyntaxError("Invalid key segment '[b]c]': characters [ and ] are not allowed"),
    );
    expect(() => serialize({ a: Object.defineProperty({}, '__proto__', { value: 'x', enumerable: true }) } as any, new URLSearchParams())).toThrow(
      new SyntaxError("Disallowed key segment '__proto__': reserved prototype property"),
    );
  });
  ...
  test('throws for blob values when serializing to URLSearchParams with exact error text', () => {
    ...
    expect(() => serialize({ file: blob }, new URLSearchParams())).toThrow(new TypeError("Key 'file': Blob values require FormData"));
  });

  test('throws for unsupported nested value types with path context', () => {
    expect(() => serialize({ a: { b: Symbol('x') as any } }, new URLSearchParams())).toThrow(new TypeError("Key 'a[b]': unsupported value type"));
  });
  ...
  test('empty objects cannot round-trip through URLSearchParams', () => {
    const result = deserialize(serialize({ config: {} }, new URLSearchParams()));
  ...
  test('empty objects inside arrays cannot round-trip through URLSearchParams', () => {
    const result = deserialize(serialize({ items: [{}] }, new URLSearchParams()));
...
describe('deserialize', () => {
  ...
  test('throws for invalid key syntax with exact error text', () => {
    expect(() => deserialize(new URLSearchParams([['[a]', 'x']]))).toThrow(
      new SyntaxError("Invalid key '[a]': expected format is 'key', 'key[]', or 'key[segment]'"),
    );
    expect(() => deserialize(new URLSearchParams([['a][b', 'x']]))).toThrow(
      new SyntaxError("Invalid key 'a][b': expected format is 'key', 'key[]', or 'key[segment]'"),
    );
    expect(() => deserialize(new URLSearchParams([['data[__proto__]', 'x']]))).toThrow(
      new SyntaxError("Disallowed key segment '__proto__': reserved prototype property"),
    );
    expect(() => deserialize(new URLSearchParams([['a[b]', 'x']]))).not.toThrow();
  });
  ...
  test('throws on conflicting path types with exact error text', () => {
    ...
    ).toThrow(new TypeError("Type conflict at segment 'data': expected object but found array"));
    ...
    ).toThrow(new TypeError("Type conflict at segment 'list': expected array but found object"));
  });
});

 ...
 */
export const toError = (data: unknown) =>
...
export const respond = {
	...
	contextError(id: string, error?: unknown) {
		this.post(id, {
			...
			status: "error",
			error: toError(
				error || "An unexpected error occurred while setting context in worker"
			)
		});
	},
...
export const eventIsRequest = (event: MessageEvent<unknown>) => {
	...
	if (
		...
		request &&
		Object.hasOwnProperty.call(request, WORKER_NAMESPACE)
	) {
		return true;
	}
	return false;

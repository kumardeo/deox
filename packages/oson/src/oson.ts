import { ARRAY_HOLE_INDEX, BIG_INT_LABEL, NAN_INDEX, NEG_INF_INDEX, PLAIN_OBJECT_LABEL, POS_INF_INDEX, UNDEFINED_INDEX } from './constants';
import { type ConstructorMap, GLOBAL_CONSTRUCTOR_MAP, getFromLabel, nearestConstructor } from './constructors';
...
const isOsonObject = (array: OsonList): array is OsonObject => typeof array[0] === 'string';

const isOsonBigInt = (array: OsonList): array is OsonBigInt => array[0] === BIG_INT_LABEL;
...
const fromObject = (value: object, constructors: ConstructorMap): [string, unknown[]] => {
...
const hydrateObject = (label: string, stub: object, val: any[], constructors: ConstructorMap) => {
  if (label === PLAIN_OBJECT_LABEL) {
...
const createObject = (label: string, val: any[], constructors: ConstructorMap) => {
  const result = getFromLabel(constructors, label);
  if (result === null) {
    throw new Error(`Unknown object type: ${label}`);
  }
 ...
 */
export const listify = (value1: unknown, constructors: ConstructorMap = GLOBAL_CONSTRUCTOR_MAP): Oson => {
 ...
 */
export function delistify(oson: Oson, constructors: ConstructorMap = GLOBAL_CONSTRUCTOR_MAP): any {
  ...
  function recover(position: number) {
    ...
    if (!(position in index)) {
      ...
      switch (typeof value) {
        // biome-ignore lint/suspicious/noFallthroughSwitchClause: fallthrough for null
        case 'object':
          if (value !== null) {
            if (isOsonBigInt(value)) {
              const val = value[1];
              const num = val.startsWith('-') ? -BigInt(`0x${val.substring(1)}`) : BigInt(`0x${val}`);
              index[position] = num;
 ...
 */
export function stringify(value: unknown = undefined, constructors: ConstructorMap = GLOBAL_CONSTRUCTOR_MAP): string {
 ...
 */
export function parse(text: string, constructors: ConstructorMap = GLOBAL_CONSTRUCTOR_MAP): any {

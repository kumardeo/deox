...
/** utils types */
export type ConstructorType<T = any> = T extends abstract new (...args: any[]) => any ? never : abstract new (...args: any[]) => T;
...
export interface ValueConstructor<C, V extends any[] = any[]> extends DecomposableConstructor<C, V> {
  /** creates a class from a value array */
  create(val: V): C;
}

export interface BucketConstructor<C, V extends any[] = any[]> extends DecomposableConstructor<C, V> {
...
export type SerializableConstructor<C, V extends any[] = any[]> = ValueConstructor<C, V> | BucketConstructor<C, V>;

export interface ConstructorMap<C = any, V extends any[] = any[]> extends Map<ConstructorType<C>, SerializableConstructor<C, V>> {
  set<T extends C, P extends V>(key: ConstructorType<T>, value: SerializableConstructor<T, P>): this;
}
...
export const globalConstructorMap = () => {
  ...
  map.set(Error, {
    from: (err) => {
      const res: [string, string, string | undefined, unknown] = [err.name, err.message, undefined, undefined];
...
export const getFromConstructor = (map: ConstructorMap, ctor: ConstructorType) => {
  let i = 0;
...
export const nearestConstructor = (map: ConstructorMap, subclass: unknown) => {
  let result: (typeof map extends Map<infer C, infer V> ? [C, V, string, number] : never) | undefined;

...
export class Transformer {
	...
	parseAttributes(attributes: NamedNodeMap): Record<string, unknown> {
		...
		for (let i = 0; i < attributes.length; i++) {
			...
			const prop =
				typeof attrToPropMap[name] === 'string' ? attrToPropMap[name] : name;
	...
	parseNode(
		input: Node,
		{ level, index, key }: { level: number; index: number; key?: Key },
	): Output {
		let node = input;
		...
		if (typeof this.modify === 'function') {
			...
			if (typeof result !== 'undefined') {
				...
				if (!(result instanceof Node)) {
					throw new TypeError(
						"Callback 'modify' must return `Node | null | undefined`",
					);
				}
				node = result;
			}
		...
		const reactKey =
			typeof key !== 'undefined' ? key : this.createKey(level, index);
		...
		if (typeof this.transform === 'function') {
			...
			if (typeof result !== 'undefined') {
				...
				if (!this.isOutput(result)) {
					throw new TypeError(
						"Callback 'transform' must return `ReactElement | string | number | bigint | boolean | null | undefined`",
					);
				}
				return result;
			}

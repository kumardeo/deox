...
export class Gumroad extends API {
	 ...
	 */
	private _compose<T extends keyof UpdateMap>(
		handlers: Handler<T>[],
		...
		return (context: Context<T>, next?: () => Promise<void>) => {
			...
			async function dispatch(i: number) {
				if (i <= index) {
					throw new SDKError(
						'Looks like next() was called multiple times. Make sure you call it only once.',
					);
				}
	 ...
	 */
	private async _dispatch<T extends keyof UpdateMap>(
		update_name: T,
		payload: UpdateMap[T],
	): Promise<void> {
	 ...
	 */
	private _formatPayload<T extends keyof UpdateMap>(
		payload: UpdateMap[T],
	): UpdateMap[T] {
		// Validate card field
		if ('card' in payload) {
	 ...
	 */
	async handle<T extends keyof UpdateMap>(
		request: Request,
		update_name: T,
	): Promise<Response> {

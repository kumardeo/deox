...
>(target: O, immutable: I, mutable?: M): O & M & Readonly<I> {
	if (mutable) {
...
export function assertNumber(
	...
	if (typeof input !== 'number') {
		throw new TypeError(
			`${name} must be of type number, current type is ${typeof input}`,
		);
	}
}
...
export function assertArray(
	...
	if (!Array.isArray(input)) {
		throw new TypeError(
			`${name} must be an Array, current type is ${typeof input}`,
		);
	}
}
...
export function assertObject(
	input: unknown,
	name: string,
): asserts input is NonNullable<object> {
...
export function formatCustomField<T extends { custom_fields: unknown }>(
	input: T,
): T {

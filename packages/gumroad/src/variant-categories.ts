...
export class VariantCategoriesMethods extends Methods {
	protected _bindVariantCategory(
		variant_category: VariantCategory,
		product_id: string,
	): VariantCategory & VariantCategoryProps {
		const properties: VariantCategoryProps = {
			update: async (title, requestOptions) =>
				this.update(product_id, variant_category.id, title, requestOptions),

			delete: async (requestOptions) =>
				this.delete(product_id, variant_category.id, requestOptions),
		};
	...
	async list(
		...
		try {
			...
			return (
				...
				this._bindVariantCategory(variant_category, product_id),
			);
		} catch (e) {
	...
	async create(
		...
		try {
			...
			this.logger.function(e, 'VariantCategories.create', {
				product_id,
				title,
			});

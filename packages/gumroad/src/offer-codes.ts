...
export class OfferCodesMethods extends Methods {
	protected _bindOfferCode(
		offer_code: OfferCode,
		product_id: string,
	): OfferCode & OfferCodeProps {
		const properties: OfferCodeProps = {
			update: async (options, requestOptions) =>
				this.update(product_id, offer_code.id, options, requestOptions),

			delete: async (requestOptions) =>
				this.delete(product_id, offer_code.id, requestOptions),
		};
	...
	async list(
		...
		try {
			...
			return (
				await this.client.request<{ offer_codes: OfferCode[] }>(
					`./products/${encodeURI(product_id)}/offer_codes`,
					{
						signal,
					},
				)
			).offer_codes.map((offer_code) =>
				this._bindOfferCode(offer_code, product_id),
			);
		} catch (e) {
	...
	async get(
		...
		try {
			...
			return this._bindOfferCode(
				(
					await this.client.request<{ offer_code: OfferCode }>(

...
export class CustomFieldsMethods extends Methods {
	protected _bindCustomField(
		custom_field: CustomField,
		product_id: string,
	): CustomField & CustomFieldProps {
		const methods: CustomFieldProps = {
			update: async (options, requestOptions) =>
				this.update(product_id, custom_field.name, options, requestOptions),

			delete: async (requestOptions) =>
				this.delete(product_id, custom_field.name, requestOptions),
		};
	...
	async list(
		...
		try {
			...
			return (
				await this.client.request<{ custom_fields: CustomField[] }>(
				...
				this._bindCustomField(custom_field, product_id),
			);
		} catch (e) {
	...
	async create(
		...
		try {
			...
			return this._bindCustomField(
				(
					await this.client.request<{ custom_field: CustomField }>(
						`./products/${encodeURI(product_id)}/custom_fields`,
						{
							method: 'POST',
							params: {
								name,
								type:
									type && ['text', 'checkbox', 'terms'].includes(type)
										? type
										: undefined,

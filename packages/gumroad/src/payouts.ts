...
export class PayoutsMethods extends Methods {
	...
	async get(
		...
		try {
			...
			return (
				await this.client.request<{ payout: Payout }>(
					`./payouts/${encodeURI(payout_id)}`,
					{

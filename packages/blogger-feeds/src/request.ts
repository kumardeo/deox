 ...
 */
const fetchJSONP = async <T = unknown>(
	getUrl: JSONPGetUrl,
	scriptOptions?: Record<string, unknown>
) => {
	(window as Record<string, any>)[JSONP_NAMESPACE] ??= queueJSONP;
...
const fetchJSON = async <T = unknown>(url: string | URL) => {
	...
	if (!contentType) {
		throw new SDKRequestError(
			`Response was success but Content-Type '${contentType}' is not supported`,
			response
		);
	}
...
export const fetchFeed = async (
	path: string | URL,
	{ params, include, exclude, baseUrl, jsonp }: FetchFeedOptions = {}
) => {
	...
	const json = await (jsonp
		? fetchJSONP(
				({ callback }) =>
					new RequestURL(endpoint, undefined, { params: { callback } })
			)
		: fetchJSON(endpoint));

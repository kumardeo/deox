...
(async () => {
	...
	await worker.proxy
		.fetchJson(
			'https://raw.githubusercontent.com/kumardeo/deox/main/packages/worker-rpc/package.json',
		)
		.then((result) => {
			console.info(
				"(thread:main) .proxy.fetchJson('..{+}../package.json'): ",
				result,
			);
		});
	...
	console.log(
		'(thread:main)',
		await worker.call([bytes.buffer], 'transferToWorker', bytes),
	);

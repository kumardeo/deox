 ...
 */
export type DrizzleKitConfig = Omit<
	Config,
	'dialect' | 'driver' | 'dbCredentials'
>;
 ...
 * );
 */
export function drizzleD1Config(
	config: DrizzleKitConfig,
	options: DrizzleD1Options = {},
): Config {
	...
	if (!database) {
		throw new Error(
			`'${usePreview ? 'preview_database_id' : 'database_id'} is not set for D1 binding '${binding.binding}'`,
		);
	}
	...
	if (!useRemote && !database.exists) {
		if (binding.databaseName) {
			...
			if (bin) {
				...
				console.log(
					`[!] SQLite file for D1 binding '${binding.binding}' does not exist.`,
				);
				console.log(
				...
				if (existsSync(database.filename)) {
					...
					console.log(
						'[✓] Command success! SQLite file has been successfully created.',
					);
				} else {
		...
		if (!database.exists) {
			throw new Error(
				`Could not find SQLite file for D1 binding '${binding.binding}'`,
			);
		}
	}

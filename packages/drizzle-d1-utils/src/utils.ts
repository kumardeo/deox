...
export function getD1BindingInfo({
	...
	if (d1_databases.length > 1 && !binding) {
		throw new Error(
			"Argument 'binding' is required when more than one D1 bindings exist in the config",
		);
	}
	...
	if (!bindingConfig.database_id && !bindingConfig.preview_database_id) {
		throw new Error(
			`Neither 'database_id' nor 'preview_database_id' is set for D1 binding '${bindingConfig.binding}'`,
		);
	}
	...
	const wranglerStateDir =
		persistTo ??
		relative('.', join(wranglerConfigDir ?? '', '.wrangler/state/v3'));

	const [database, previewDatabase] = [
		bindingConfig.database_id,
		bindingConfig.preview_database_id,
	].map((databaseId) => {
		if (!databaseId) {
			return null;
		}
